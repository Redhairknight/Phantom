...
(function ($) {
    ...
    var DanmuPlayer = function (element, options) {
        ...
        this.$ctrlMain.append('<div class="full-screen   ctrl-btn-right"><span class="" aria-hidden="true">Full Screen</span></div>');
        this.$ctrlMain.append('<div class="show-danmu  ctrl-btn-right ctrl-btn-right-active"><span class="" aria-hidden="true">Close Bullet</span></div>');
        ...
        $(this.id + " .full-screen").on("click", {video: this.video, that: that}, function (e) {
            if (!e.data.that.danmuPlayerFullScreen) {
				...
				$(e.data.that.id + " .full-screen span").html('Exit full Screen');
            }
            else {
                $(e.data.that.id).removeClass("danmu-player-full-screen");
                e.data.that.danmuPlayerFullScreen = false;
				$(e.data.that.id + " .full-screen span").html('Full screen');
            }
        ...
        $(this.id + " .show-danmu").on("click", {that: that}, function (e) {
            if (e.data.that.danmuShowed) {
				...
				$(e.data.that.id + " .show-danmu").html('Open Bullet');
            }
            else {
                e.data.that.danmuShowed = true;
                $(e.data.that.id + " .danmu-div").css("visibility", "visible");
				$(e.data.that.id + " .show-danmu").html('Close Bullet');
            }
